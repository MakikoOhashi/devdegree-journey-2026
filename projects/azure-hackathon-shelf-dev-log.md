...
## 2026-05-16
- `pending clarification queue`（保留状態の受け皿）を追加
  - AI が「情報不足で止まる」ことを前提に、止まった案件を溜めて人間へ返す導線を設計に入れた

## 2026-05-18
- 承認センターと確認ページを統合（左右カラム）
  - 右で投入 → すぐ左に反映され Issue 化される流れを一画面で可視化
  - 状態遷移・入力・Issue化・Human確認が分断されると理解コストが急増するため、「業務の流れが見える」UI を優先

...
- LeetCode を1問完了
- 学び:
  - `increment: () => ++current` は「引数なしで呼ばれたら、外側の `current` を1増やしてその値を返す関数」
  - 外側の `current` を内側関数が保持し続ける = closure + state 管理
  - `++current` は prefix increment（増やした後の値を返す）
- つまずき:
  - 「関数が値を返す」のではなく「呼ぶたびに共有 state が変わる」点がまだ腹落ちしていない
- 次アクション:
  - closure / state 変化を図（外枠 `current` → 内側 `increment` → 呼ぶたび更新）で理解してから同型をもう1問
